...
const collectEmployees = function(employeesArray) {
  // User inputs the name of the employee and the salary, and ask if you want to add another one. 
  let k=true;
  let employees = [];
  while (k == true){
    let employee = [];
...
const displayAverageSalary = function(employeesArray) {
  ...
  console.log(`The average employee salary of ${employeesArray.length} employee(s) is $${averageSalary}`);
}

// Select a random employee
const getRandomEmployee = function(employeesArray) {
  // Take a random number from the employeesArray and get the firstName and lastName
  let random = Math.floor(Math.random()*employeesArray.length);
  let employeeFirstName = employeesArray[random].firstName;
  let employeeLastName = employeesArray[random].lastName;
